...
# Can be turned on/off from the commandline by setting env variables:
# (Neither is used by ClassHierarchy)
IS_TEST_SERIALIZABLES ?= 0
IS_MAKE_UML ?= 0

# ==================================================================
# Internally used variables:

# Dependency packages are in the directory above the package
# hierarchy (n.b. there are none for ClassHierarchy).
PLATFORM_DIR = ..

# Refer to packages in Makefiles as "x/y/z", but in Java commands
# as "x.y.z".
JAVA_PACKAGE = \
	$(subst /,.,$(PACKAGE))

# Refer to classes in Makefiles as "XYZ.class", but in Java
# commands as just "XYZ".
JAVA_CLASSES = \
...
#
# If javac is not in your PATH, this will fail.  :(
#
COMPILE_BIN = javac
COMPILE_PRE = \
	-Xlint:all \
...
#
# If javadoc is not in your PATH, this will fail.  :(
#
DOC_BIN = javadoc
DOC_PRE = \
...
#
# If jar is not in your PATH, this will fail.  :(
#
LIB_BIN = jar
LIB_PRE = \
...
#
# If java is not in your PATH, this will fail.  :(
#
BIN_BIN = java
BIN_PRE = \
	-classpath $(CLASSPATH_COMPILE):$(CLASSPATH_DEPENDENCIES)
...
DIST_PRE = 
DIST = \
	$(PACKAGE_FILENAME).$(VERSION).tar.gz
PROFILE_POST = 
...
#
# If rmic is not in your PATH, this will fail.  :(
#
.PHONY: rmi_compile
rmi_compile:
	rmic \
...
remove_dist_file:
	...
	do \
	  if test ! -f $(BUILD_DIR)/lib/$$FILE_OR_DIR; \
	  then \
	    continue; \
	  fi; \
	  $(REMOVE_DIRS) $(BUILD_DIR)/lib/$$FILE_OR_DIR; \
	  if test $$? -ne 0; \
	  then \
	...
	do \
	  if test ! -f $(BUILD_DIR)/bin/$$FILE_OR_DIR; \
	  then \
	    continue; \
	  fi; \
	  $(REMOVE_DIRS) $(BUILD_DIR)/bin/$$FILE_OR_DIR; \
	  if test $$? -ne 0; \
	  then \
